...
RECORD_FLAG = '[dat]'
RESULT_DIR = '../textMatchContest_trainData_30w/'
INPUT_FILEPATH = './final_total_data_train_tab.tsv'
...
def generateStanderKeyValue(result_dir, input_data, vocabulary_filepath, is_training=True):
    ...
    for i, record in tqdm(enumerate(total_records)):
        ...
        if label not in ('0', '1'):
            ...
            print(record)
            continue
        if record[3] not in categorylist:
            print('Find a broken data , category not in (0 ,1),')
            print(record)
            continue
        category = category2id[record[3]]
        ...
        # convert to str
        record_list = list()
        keys = ['question1', 'question2', 'input_ids', 'segment_ids',
                'input_masks', 'label', 'category', 'category_ids']
        values = [question1, question2, input_ids_parse, segment_ids, input_masks, label, record[3], category]
