...
BERT_CLS = '101'
BERT_SEP = '102'
BERT_INPUT_WORD_LEN = 256
RECORD_LEN = 6
BERT_INPUT_WORD_LEN_EXCLUDE_CLS_SEP = BERT_INPUT_WORD_LEN - 2
RECORD_FLAG = '[dat]'
RESULT_DIR = '../queryContentData/'
INPUT_FILEPATH = './med_query_title_answer.txt'
...
def parse_input(query_ids, content_ids):
...
def generateStanderKeyValue(result_dir, input_data, vocabulary_filepath, is_training=True):
    ...
    for i, record in tqdm(enumerate(total_records)):
        """ ignore | label1 | label2 | score | query | content"""
        ...
        content_ids = [convert2id(ch) for ch in list(content.strip())]

        # parse input_ids
        input_ids_parse, segment_ids, input_masks = parse_input(query_ids, content_ids)
        # convert to str
        record_list = list()
        keys = ['query', 'content', 'input_ids', 'segment_ids', 'input_masks', 'label']
        values = [query, content, input_ids_parse, segment_ids, input_masks, label]
